...
class User
{
	...
	public function createUserAccount($username, $email, $password, $usertype)
	{
		...
		if($this->emailExists($email)){
			...
			$pre_stmt->bind_param("sssssss",$username,$email,$pass_hash,$usertype, $register_date, $last_login, $notes);
	...
	public function userLogin($email, $password)
	{
		...
		if($result->num_rows < 1 ){
			...
			if(password_verify($password,$row["password"])){
				...
				// Update user last login time when performing login
...
// $user = new User();
// echo $user->createUserAccount("Imon","contact@imonweb.co.uk","p@ssw0rd","Admin");

// echo $user->userLogin("contact@imonweb.co.uk","p@ssw0rd");
